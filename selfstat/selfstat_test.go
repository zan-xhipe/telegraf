...
func TestRegisterAndIncrAndSet(t *testing.T) {
	...
	assert.Equal(t, "self_test", foo.Name())
}
...
func TestRegisterMetricsAndVerify(t *testing.T) {
	...
	// TODO test that the metrics are all correct
	acc := testutil.Accumulator{}
	acc.AddMetrics(Metrics())

	// verify s1 & s2
	acc.AssertContainsTaggedFields(t, "self_test_timing",
	...
	// verify s5
	acc.AssertContainsTaggedFields(t, "self_test",
		map[string]interface{}{
